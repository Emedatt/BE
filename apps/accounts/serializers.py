    ...
    def validate_medical_history(self, value):
        if not isinstance(value, dict):
            raise serializers.ValidationError(
                "Medical history must be a valid JSON object"
            )
        return value
...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if data["role"] == "admin":
            raise serializers.ValidationError(
                {"role": "Admin role cannot be created via this endpoint"}
            )
    ...
    def create(self, validated_data):
        ...
        if user.role == "patient":
            ...
            DoctorProfile.objects.create(
                user=user,
                license_number=f"TEMP-LIC-{uuid.uuid4().hex[:8]}",
                specialty="General",
            )
...
class AdminUserCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if data["role"] != "admin":
            raise serializers.ValidationError(
                {"role": "This endpoint is for creating admin users only"}
            )
        return data
    ...
    def validate(self, data):
        ...
        try:
            token = PasswordResetToken.objects.get(
                token=data["token"], expires_at__gt=timezone.now()
            )
