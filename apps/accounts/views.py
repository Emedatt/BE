    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                ...
                "role": openapi.Schema(
                    type=openapi.TYPE_STRING, enum=["patient", "doctor"]
                ),
            },
        ),
        responses={
...
class LoginView(APIView):
    ...
    * Rate limited to prevent brute force attacks
    ...
    def post(self, request):
        ...
        log_audit(
            None, "login_failed", {"email": email, "reason": "invalid_credentials"}
        )
        return Response(
...
class SessionListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        user_sessions = [
            s
            for s in sessions
            if s.get_decoded().get("_auth_user_id") == str(self.request.user.id)
        ]
    ...
    def delete(self, request, id):
        try:
            session = Session.objects.get(
                session_key=id, expire_date__gt=timezone.now()
            )
            if session.get_decoded().get("_auth_user_id") == str(request.user.id):
                ...
                log_audit(
                    request.user,
                    "session_terminated",
                    {"email": request.user.email, "session_id": id},
                )
