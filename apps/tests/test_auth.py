...
def verified_doctor():
    ...
    DoctorProfile.objects.create(
        user=user, license_number="TEST-123", specialty="General"
    )
    return user
...
class TestPasswordReset:
    def test_password_reset_request(self, api_client, verified_patient):
        response = api_client.post(
            reverse("accounts:password-reset"), {"email": verified_patient.email}
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_password_reset_confirm(self, api_client, verified_patient):
        token = PasswordResetToken.objects.create(
            user=verified_patient, expires_at=timezone.now() + timedelta(hours=24)
        )
        data = {
            "token": token.token,
        ...
        response = api_client.post(
            reverse("accounts:password-reset-confirm", args=[token.token]), data
        )
        assert response.status_code == status.HTTP_200_OK
